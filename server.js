// server.js
// Backend Foundry Invest Platform (Versão Consolidada e Atualizada)
...
const ADMIN_NAME = process.env.ADMIN_NAME || 'Admin Foundry'; // Nome atualizado
...
const DEFAULT_MIN_WITHDRAWAL = parseFloat(process.env.DEFAULT_MIN_WITHDRAWAL) || 100; // Atualizado
...
// --- Middlewares Globais ---
app.use(cors()); // Habilita CORS para todas as origens ANTES das rotas.
...
const UserSchema = new mongoose.Schema({
    ...
    canWithdrawBonus: { type: Boolean, default: false }, // NOVO: Para controlar saque de bônus
    status: { type: String, enum: ['active', 'pending_verification', 'suspended', 'banned'], default: 'active'}, //
    lastLoginAt: { type: Date }, //
    failedLoginAttempts: { type: Number, default: 0 }, //
    lockUntil: { type: Date }, //
    passwordResetToken: { type: String }, // Para fluxo tradicional de reset de senha
    passwordResetExpires: { type: Date }, //
    oneTimeLoginToken: { type: String }, // NOVO: Para link de login único
    oneTimeLoginTokenExpires: { type: Date }, // NOVO: Expiração do OTLT
...
UserSchema.methods.createPasswordResetToken = function() { //
    const resetToken = crypto.randomBytes(20).toString('hex');  // Alterado para 20 bytes para ser um pouco mais curto que OTLT
    this.passwordResetToken = crypto.createHash('sha256').update(resetToken).digest('hex'); //
    this.passwordResetExpires = Date.now() + 15 * 60 * 1000;  //
    return resetToken; //
};
UserSchema.methods.createOneTimeLoginToken = function() { //
    ...
    return loginToken; // Retorna o token não hasheado para ser usado no link
};
...
const SystemSettingsSchema = new mongoose.Schema({
    ...
    bonusWithdrawalRequiresPlan: { type: Boolean, default: true }, // NOVO: Configuração para saque de bônus
...
const DepositMethodSchema = new mongoose.Schema({
    ...
    accountDetailsSchema: { type: mongoose.Schema.Types.Mixed }, // Estrutura dos campos que o admin preenche para este método
    paymentInfo: { type: mongoose.Schema.Types.Mixed, required: true }, // Detalhes concretos da conta da empresa
...
const TransactionSchema = new mongoose.Schema({
    ...
    type: {  //
        ...
        enum: [
            ...
            'admin_credit', 'admin_debit', 'withdrawal_fee', 'admin_plan_assignment', // admin_plan_assignment ADICIONADO
            'other_fee' //
        ], 
        required: true 
    },
...
const PlanSchema = new mongoose.Schema({
    ...
    isActive: { type: Boolean, default: true, index: true }, // Adicionado index
    features: [String],  //
...
const UserInvestmentSchema = new mongoose.Schema({
    ...
    lastCollectedAt: {type: Date }, // Novo campo
    createdAt: { type: Date, default: Date.now } //
});
...
const PromotionSchema = new mongoose.Schema({
    ...
    type: {type: String, enum: ['banner', 'popup', 'general', 'blog'], default: 'general'}, // 'blog' adicionado
...
// NOVO SCHEMA para Contadores Regressivos Independentes
...
const protectRoute = (req, res, next) => { /* ... como antes ... */  //
    ...
    if (!token) { return res.status(401).json({ m: 'Acesso negado. Token não fornecido.' }); }
    try {
        const decoded = jwt.verify(token, JWT_SECRET);
        req.user = decoded.user; next();
    } catch (err) {
        if (err.name === 'TokenExpiredError') return res.status(401).json({ m: 'Token expirado.' });
        if (err.name === 'JsonWebTokenError') return res.status(401).json({ m: 'Token inválido.' });
        console.error("Erro na verificação do token:", err);
        return res.status(500).json({ m: 'Erro ao verificar token.' });
    }
};
...
const adminOnly = async (req, res, next) => { /* ... como antes ... */  //
    try {
        if (req.user && req.user.id) {
            ...
            else { res.status(403).json({ m: 'Acesso negado. Apenas administradores.' }); }
        } else { res.status(401).json({ m: 'Não autorizado.' }); }
    } catch(error) { console.error("Erro na verificação de admin:", error); res.status(500).json({ m: "Erro ao verificar permissões de administrador."}); }
};

async function getOrInitializeSystemSettings() { /* ... como antes, com bonusWithdrawalRequiresPlan ... */  //
    try {
        let settings = await SystemSettings.findOne({ singletonId: 'main_settings' }); //
        if (!settings) {
            console.log('Nenhuma configuração do sistema Foundry encontrada, inicializando com padrões...'); //
            ...
            await settings.save(); //
            console.log('Configurações do sistema Foundry inicializadas com sucesso.'); //
        }
        return settings; //
    } catch (error) { console.error("Erro ao obter/inicializar configurações do sistema Foundry:", error.message); throw new Error("Falha ao carregar as configurações do sistema."); } //
}

async function createInitialAdmin() { /* ... como antes, com nome Foundry e canWithdrawBonus ... */ //
    try {
        if (!ADMIN_EMAIL || !ADMIN_PASSWORD) { console.warn("Credenciais do administrador padrão Foundry não definidas no .env. Admin não será criado."); return; } //
        ...
        if (!adminExists) {
            const adminUser = new User({ //
                ...
                canWithdrawBonus: true // Admin pode sacar bônus por padrão
            });
            await adminUser.save(); console.log('Usuário administrador inicial da Foundry criado com sucesso!'); //
        }
    } catch (error) { console.error('Erro ao criar administrador inicial da Foundry:', error.message); } //
}
...
// --- ROTAS DA API ---
// -----------------------------------------------------------------------------
app.get('/api', (req, res) => res.json({ message: 'API Foundry Invest Funcionando!' })); // Rota de teste

// --- Rotas de Autenticação (/api/auth) ---
const authRouter = express.Router(); //
authRouter.post('/register', async (req, res) => { /* ... como antes, com nome Foundry ... */  //
    try {
        ...
        if (registrationBonusApplied > 0) { //
            await createTransactionEntry(newUser._id, 'registration_bonus', registrationBonusApplied, 'Bônus de Registro Foundry', 'completed', 0, newUser.balance); // Nome atualizado
            await createUserNotification(newUser._id, 'Bem-vindo à Foundry Invest!', `Você recebeu um bônus de registro de ${registrationBonusApplied.toFixed(2)} MT!`, 'success', '/wallet'); // Nome atualizado
        }
        if (referredByUser) { await createUserNotification(referredByUser._id, 'Nova Indicação na Foundry!', `${newUser.name} registrou-se usando seu código de indicação.`, 'info', '/referrals'); } // Nome atualizado
        res.status(201).json({ m: 'Usuário registrado com sucesso!', userId: newUser._id }); //
    } catch (error) { console.error("[Foundry] Erro no registro de usuário:", error); if (error.name === 'ValidationError') { return res.status(400).json({ m: "Erro de validação nos dados fornecidos.", e: Object.values(error.errors).map(val => val.message) }); } res.status(500).json({ m: 'Ocorreu um erro no servidor ao tentar registrar o usuário.' }); } //
});
...
// ATUALIZADO: Verifica resposta e retorna token de login único (OTLT)
authRouter.post('/recover/verify-answer-and-get-login-link', async (req, res) => { //
    try { //
        ...
        console.log(`[Foundry][AUTH] Token de login único gerado para ${user.email} (para admin enviar ao usuário): ${oneTimeLoginToken}`); //
        res.json({ //
            m: "Resposta verificada. Um link de login único pode ser gerado com o token fornecido (válido por 15 minutos).", //
            oneTimeLoginToken: oneTimeLoginToken  // Envia o token para o admin construir o link
        });
...
userRouter.get('/profile', async (req,res)=>{ /* ... como antes ... */  //
    ...
    } catch (error) { console.error("[Foundry] Erro ao buscar perfil:", error); res.status(500).json({ m: "Erro ao buscar informações do perfil." }); } //
});
userRouter.put('/profile', async (req,res)=>{ /* ... como antes ... */  //
    ...
    } catch (error) { console.error("[Foundry] Erro ao atualizar perfil:", error); if (error.name === 'ValidationError') { return res.status(400).json({ m: "Dados inválidos.", e: Object.values(error.errors).map(val => val.message) }); } res.status(500).json({ m: "Erro ao atualizar perfil." }); } //
});
userRouter.put('/change-password', async (req,res)=>{ /* ... como antes ... */  //
    ...
    } catch (error) { console.error("[Foundry] Erro ao alterar senha:", error); res.status(500).json({ m: "Erro ao alterar senha." }); } //
});
userRouter.get('/referral-details', async (req,res)=>{ /* ... como antes ... */  //
    ...
    } catch (error) { console.error("[Foundry] Erro ao buscar detalhes de indicação:", error); res.status(500).json({ m: "Erro ao buscar detalhes de indicação." }); } //
});
userRouter.get('/transactions', async (req, res) => { /* ... como antes ... */  //
    ...
    } catch (error) { console.error("[Foundry] Erro ao buscar transações:", error); res.status(500).json({ m: "Erro ao buscar transações." }); } //
});
userRouter.get('/notifications', async (req, res) => { /* ... como antes ... */  //
    ...
    } catch (error) { console.error("[Foundry] Erro ao buscar notificações:", error); res.status(500).json({ m: "Erro ao buscar notificações." }); } //
});
userRouter.put('/notifications/:id/read', async (req, res) => { /* ... como antes ... */  //
    ...
    } catch (error) { console.error("[Foundry] Erro ao marcar notificação como lida:", error); res.status(500).json({ m: "Erro ao atualizar notificação." }); } //
});
userRouter.put('/notifications/read-all', async (req, res) => { /* ... como antes ... */  //
    ...
    } catch (error) { console.error("[Foundry] Erro ao marcar todas notificações como lidas:", error); res.status(500).json({ m: "Erro ao atualizar notificações." }); } //
});
...
// --- ROTAS ADMIN: CRUD PARA PLANOS (IMPLEMENTADAS) ---
adminRouter.post('/plans', async (req, res) => { /* ... como antes ... */  //
    try {
...
adminRouter.get('/plans/all', async (req, res) => { /* ... como antes ... */  //
    try {
...
adminRouter.get('/plans/:id', async (req, res) => { /* ... como antes ... */  //
    try {
        ...
        if (!plan) { //
            console.warn(`[Foundry][ADMIN] Plano com ID ${req.params.id} não encontrado.`); //
            return res.status(404).json({ m: "Plano não encontrado." }); //
        }
        res.json(plan); //
    } catch (error) { //
        console.error("[Foundry] Erro ao buscar plano por ID (admin):", error); //
        res.status(500).json({ m: "Erro ao buscar plano." }); //
    }
});

adminRouter.put('/plans/:id', async (req, res) => { /* ... como antes, com parse de features ... */  //
    try {
        if (!mongoose.Types.ObjectId.isValid(req.params.id)) return res.status(400).json({ m: "ID de plano inválido." }); //
        
        const updateData = { ...req.body }; //
        if (updateData.features && typeof updateData.features === 'string') { //
            updateData.features = updateData.features.split(',').map(f => f.trim()).filter(f => f); // Filtra strings vazias também
        } else if (updateData.features && !Array.isArray(updateData.features)) { //
            delete updateData.features; // Evita erro se não for array ou string que possa ser convertida
        }

        if (updateData.isActive !== undefined) { //
            updateData.isActive = (updateData.isActive === true || String(updateData.isActive).toLowerCase() === 'true'); //
        }
        ...
        ['price_mt', 'daily_profit_mt', 'duration_days', 'hashrate_mhs', 'maxInvestmentsPerUser'].forEach(field => { //
            ...
            } else if (updateData[field] === '') { // Se campo numérico foi enviado vazio, remove para não dar erro de cast ou NaN
        ...
        if (!updatedPlan) { //
            console.warn(`[Foundry][ADMIN] Plano com ID ${req.params.id} não encontrado para atualização.`); //
            return res.status(404).json({ m: "Plano não encontrado para atualização." }); //
        }
        console.log("[Foundry][ADMIN] Plano atualizado:", updatedPlan.name); //
        res.json({ m: "Plano atualizado com sucesso!", plan: updatedPlan }); //
    } catch (error) { //
        console.error("[Foundry] Erro ao atualizar plano (admin):", error); //
        if (error.code === 11000) return res.status(400).json({m: "Um plano com este nome já existe."}); //
        if (error.name === 'ValidationError') return res.status(400).json({m: "Erro de validação.", e: Object.values(error.errors).map(val => val.message)}); //
...
adminRouter.delete('/plans/:id', async (req, res) => { /* ... como antes ... */  //
    try {
        if (!mongoose.Types.ObjectId.isValid(req.params.id)) return res.status(400).json({ m: "ID de plano inválido." }); //
        
        const activeInvestments = await UserInvestment.countDocuments({ plan: req.params.id, isActive: true }); //
        if (activeInvestments > 0) { //
            return res.status(400).json({m: `Não é possível deletar. Existem ${activeInvestments} investimentos ativos usando este plano. Desative-o primeiro.`}); //
        }
        console.log(`[Foundry][ADMIN] Deletando plano ID ${req.params.id}`); //
        const deletedPlan = await Plan.findByIdAndDelete(req.params.id); //
        if (!deletedPlan) { //
            console.warn(`[Foundry][ADMIN] Plano com ID ${req.params.id} não encontrado para deleção.`); //
            return res.status(404).json({ m: "Plano não encontrado para deleção." }); //
        }
        console.log("[Foundry][ADMIN] Plano deletado:", deletedPlan.name); //
        res.json({ m: "Plano deletado com sucesso." }); //
    } catch (error) { //
        console.error("[Foundry] Erro ao deletar plano (admin):", error); //
        res.status(500).json({ m: "Erro no servidor ao deletar plano." }); //
    }
});

// Rota Admin para atribuir plano a usuário
adminRouter.post('/users/:userId/assign-plan', async (req, res) => { /* ... como antes ... */  //
    try {
        ...
        if (userInvestmentCount === 1 && !user.canWithdrawBonus) { user.canWithdrawBonus = true; await user.save(); } //
...
// Rota Admin para iniciar reset de senha tradicional
adminRouter.post('/users/:userId/initiate-password-reset', async (req, res) => { //
    try { //
        ...
        const user = await User.findById(userId); //
        if (!user) return res.status(404).json({m: "Usuário não encontrado."}); //
        const resetToken = user.createPasswordResetToken(); // Usa o método tradicional
        await user.save({ validateBeforeSave: false }); //
        ...
        res.json({m: `Processo de redefinição de senha iniciado para ${user.email}. Token (para admin): ${resetToken}`}); //
    } catch (error) { console.error("[Foundry] Erro ao iniciar reset de senha pelo admin:", error); res.status(500).json({m: "Erro no servidor."}); } //
});


// Rotas de Admin existentes (settings, users, deposit-methods, deposit-requests, withdrawal-requests, investments, stats)
adminRouter.get('/settings',async(req,res)=>{ /* ... como antes ... */ try{const s=await SystemSettings.findOne({singletonId:'main_settings'});res.json(s);}catch(e){res.status(500).json({m:"Erro ao buscar configurações."})} }); //
...
adminRouter.post('/countdowns', async (req, res) => { //
    ...
    } catch (error) { console.error("[Foundry] Erro ao criar contador:", error); if (error.name === 'ValidationError') return res.status(400).json({m: "Dados inválidos.", e: Object.values(error.errors).map(v=>v.message)}); res.status(500).json({ m: "Erro no servidor ao criar contador." }); } //
});
adminRouter.get('/countdowns/all', async (req, res) => { //
    ...
    } catch (error) { console.error("[Foundry] Erro ao listar contadores (admin):", error); res.status(500).json({ m: "Erro ao buscar contadores." }); } //
});
adminRouter.put('/countdowns/:id', async (req, res) => { //
    ...
    } catch (error) { console.error("[Foundry] Erro ao atualizar contador:", error); if (error.name === 'ValidationError') return res.status(400).json({m: "Dados inválidos.", e: Object.values(error.errors).map(v=>v.message)}); res.status(500).json({ m: "Erro no servidor ao atualizar contador." }); } //
});
adminRouter.delete('/countdowns/:id', async (req, res) => { //
    ...
    } catch (error) { console.error("[Foundry] Erro ao deletar contador:", error); res.status(500).json({ m: "Erro no servidor ao deletar contador." }); } //
});
...
publicPlanRouter.get('/',async(req,res)=>{ /* ... como antes ... */  //
    ...
    } catch (error) { console.error("[Foundry] Erro ao buscar planos públicos:", error); res.status(500).json({ m: "Erro ao buscar planos." }); } //
});
publicPlanRouter.get('/:id',async(req,res)=>{ /* ... como antes ... */  //
    ...
    } catch (error) { console.error("[Foundry] Erro ao buscar plano público por ID:", error); res.status(500).json({ m: "Erro ao buscar plano." }); } //
});
...
publicDepositMethodRouter.get('/', async(req,res)=>{ /* ... como antes ... */  //
    ...
    } catch (error) { console.error("[Foundry] Erro ao buscar métodos de depósito públicos:", error); res.status(500).json({ m: "Erro ao buscar métodos de depósito." }); } //
});
app.use('/api/deposit-methods', publicDepositMethodRouter); // Não protegido, para usuários verem antes de solicitar

const blogRouter = express.Router(); //
// Rotas de blog (POST, GET /all, GET /id/:id, PUT /:id, DELETE /:id são adminOnly)
blogRouter.post('/', protectRoute, adminOnly, async (req, res) => { /* ... como antes ... */  //
    try {
        ...
        const existingSlug = await BlogPost.findOne({ slug: postSlug }); if (existingSlug && (!req.params.id || existingSlug._id.toString() !== req.params.id)) return res.status(400).json({m: "Este slug já está em uso. Escolha outro."}); //
        const newPost = new BlogPost({ title, content, slug: postSlug, snippet: snippet || (content.length > 250 ? content.substring(0, 250) + '...' : content), tags: Array.isArray(tags) ? tags : (tags ? tags.split(',').map(t=>t.trim()) : []), isPublished: isPublished === true, coverImageUrl, author: req.user.id }); //
...
blogRouter.get('/', async (req, res) => { /* ... rota pública como antes ... */  //
    try {
        const page = parseInt(req.query.page) || 1; const limit = parseInt(req.query.limit) || 10; const skip = (page - 1) * limit; //
...
blogRouter.get('/all', protectRoute, adminOnly, async (req, res) => { /* ... rota admin como antes ... */  //
    try {
...
blogRouter.get('/slug/:slug', async (req, res) => { /* ... rota pública como antes ... */  //
    try {
...
blogRouter.get('/id/:id', protectRoute, adminOnly, async (req, res) => { /* ... rota admin como antes ... */  //
    try {
...
blogRouter.put('/:id', protectRoute, adminOnly, async (req, res) => { /* ... como antes ... */  //
    ...
    } catch (error) { console.error("[Foundry] Erro ao atualizar post:", error); if (error.code === 11000) return res.status(400).json({m: "Um post com este título ou slug já existe."}); if (error.name === 'ValidationError') return res.status(400).json({m: "Erro de validação.", e: Object.values(error.errors).map(val => val.message)}); res.status(500).json({ m: "Erro no servidor ao atualizar post." }); } //
});
blogRouter.delete('/:id', protectRoute, adminOnly, async (req, res) => { /* ... como antes ... */  //
    ...
    } catch (error) { console.error("[Foundry] Erro ao deletar post:", error); res.status(500).json({ m: "Erro no servidor ao deletar post." }); } //
});
app.use('/api/blog', blogRouter); //

const promotionRouter = express.Router(); //
// Rotas de promoção (POST, GET /all, GET /type/:typeName, GET /:id, PUT /:id, DELETE /:id são adminOnly ou mistas)
...
promotionRouter.get('/active', async (req, res) => { /* ... como antes (rota pública) ... */  //
    try {
        const now = new Date(); //
...
promotionRouter.get('/type/:typeName', async (req, res) => { // Rota pública para tipos específicos
    try { //
      ...
      const activePromotionsByType = await Promotion.find({ //
        type: typeName, isActive: true, //
        $or: [ { startDate: { $lte: now } }, { startDate: null } ], //
        $or: [ { endDate: { $gte: now } }, { endDate: null } ] //
      }).sort({ priority: -1, createdAt: -1 }); //

      if (!activePromotionsByType) { //
        console.log(`[Foundry][PROMO] Nenhuma promoção encontrada para o tipo ${typeName}.`); //
        return res.status(200).json([]); //
      }
      console.log(`[Foundry][PROMO] Encontradas ${activePromotionsByType.length} promoções do tipo ${typeName}.`); //
      res.json(activePromotionsByType); //
    } catch (error) { //
      console.error(`[Foundry][PROMO] Erro ao buscar promoções ativas por tipo (${req.params.typeName}):`, error); //
      res.status(500).json({ m: "Erro ao buscar promoções por tipo." }); //
    }
});
promotionRouter.get('/all', protectRoute, adminOnly, async (req, res) => { /* ... rota admin como antes ... */  //
    try {
        const page = parseInt(req.query.page) || 1; const limit = parseInt(req.query.limit) || 10; const skip = (page - 1) * limit; //
...
promotionRouter.get('/:id', protectRoute, adminOnly, async (req, res) => { /* ... rota admin como antes ... */  //
    try {
...
countdownRouter.post('/', protectRoute, adminOnly, async (req, res) => { /* ... como definido antes ... */  //
    try {
        const { title, description, targetDate, isActive, actionLink, displayLocation } = req.body; //
        if (!title || !targetDate) { return res.status(400).json({ m: "Título e Data Alvo são obrigatórios para o contador." }); } //
        const newCountdown = new Countdown({ title, description, targetDate: new Date(targetDate), isActive, actionLink, displayLocation }); //
...
countdownRouter.get('/all', protectRoute, adminOnly, async (req, res) => { /* ... como definido antes ... */  //
    try { const countdowns = await Countdown.find().sort({ targetDate: -1 }); res.json(countdowns); //
    } catch (error) { console.error("[Foundry] Erro ao listar contadores (admin):", error); res.status(500).json({ m: "Erro ao buscar contadores." }); } //
});
// Admin: Atualizar contador
countdownRouter.put('/:id', protectRoute, adminOnly, async (req, res) => { /* ... como definido antes ... */  //
    try { //
        if(!mongoose.Types.ObjectId.isValid(req.params.id)) return res.status(400).json({m:"ID de contador inválido."}); //
        const updateData = { ...req.body }; //
        if (updateData.targetDate) updateData.targetDate = new Date(updateData.targetDate); //
...
countdownRouter.delete('/:id', protectRoute, adminOnly, async (req, res) => { /* ... como definido antes ... */  //
    try { //
        if(!mongoose.Types.ObjectId.isValid(req.params.id)) return res.status(400).json({m:"ID de contador inválido."}); //
        const deletedCountdown = await Countdown.findByIdAndDelete(req.params.id); //
        if (!deletedCountdown) return res.status(404).json({m: "Contador não encontrado para deleção."}); //
...
// Montar o countdownRouter em /api/countdowns para as rotas públicas E em /api/admin/countdowns para as rotas de admin
// As rotas dentro do countdownRouter já têm a proteção adminOnly onde necessário.
app.use('/api/countdowns', countdownRouter); //
app.use('/api/admin/countdowns', countdownRouter); // As rotas de admin dentro do router já são protegidas.
...
depositUserRouter.post('/request', async (req, res) => { /* ... como antes ... */  //
    ...
    } catch (error) { console.error("[Foundry] Erro ao criar solicitação de depósito:", error); res.status(500).json({ m: "Erro ao processar sua solicitação de depósito." }); } //
});
...
withdrawalUserRouter.post('/request', async (req, res) => { /* ... como antes, com a verificação de bônus ... */  //
    try { //
        ...
        if (systemSettings.bonusWithdrawalRequiresPlan && !user.canWithdrawBonus) { //
            // Se canWithdrawBonus for false e a configuração exigir plano, verifica se o saque é maior que o saldo real (saldo - bônus elegíveis)
            // Esta lógica é complexa sem um campo `bonusBalance`. Simplificando: se o bônus de registro foi o único crédito e nenhum plano foi comprado.
            const registrationBonus = await Transaction.findOne({user: user._id, type: 'registration_bonus'});
            const totalInvestments = await UserInvestment.countDocuments({user: user._id});
            if (registrationBonus && totalInvestments === 0 && parsedAmount > (user.balance - registrationBonus.amount)) {
                 return res.status(400).json({ m: "Você precisa ativar um plano de investimento para poder sacar valores que incluem seu bônus de registro." }); //
            }
        }

        const feeCharged = parsedAmount * systemSettings.withdrawalFeePercentage; //
        // O valor que realmente sai do saldo do usuário é o `parsedAmount`. A `feeCharged` é descontada do que ele recebe.
    ...
    } catch (error) { console.error("[Foundry] Erro ao criar solicitação de saque:", error); res.status(500).json({ m: "Erro ao processar sua solicitação de saque." }); } //
});
...
investmentRouter.post('/', protectRoute, async (req, res) => { //
    console.log("[Foundry][INVEST] Rota POST /api/investments acessada"); //
    try { //
        const { planId } = req.body; console.log(`[Foundry][INVEST] Tentando investir no planId: ${planId}`); if (!planId || !mongoose.Types.ObjectId.isValid(planId)) { console.warn("[Foundry][INVEST] ID do plano inválido:", planId); return res.status(400).json({m:"ID do plano inválido."}); } //
        console.log("[Foundry][INVEST] Buscando plano..."); const plan = await Plan.findOne({ _id: planId, isActive: true }); if (!plan) { console.warn(`[Foundry][INVEST] Plano não encontrado ou inativo: ${planId}`); return res.status(404).json({m:"Plano não encontrado ou está inativo."}); } console.log("[Foundry][INVEST] Plano encontrado:", plan.name); //
        console.log("[Foundry][INVEST] Buscando usuário:", req.user.id); const user = await User.findById(req.user.id); if (!user) { console.error("[Foundry][INVEST] Usuário não encontrado no DB. ID:", req.user.id); return res.status(404).json({m:"Usuário não encontrado."}); } console.log(`[Foundry][INVEST] Usuário ${user.name} encontrado. Saldo: ${user.balance}`); if (user.balance < plan.price_mt) { console.warn(`[Foundry][INVEST] Saldo insuficiente: ${user.balance}, Preço: ${plan.price_mt}`); return res.status(400).json({m:"Saldo insuficiente para adquirir este plano."}); } //
        if (plan.maxInvestmentsPerUser > 0) { console.log(`[Foundry][INVEST] Verificando maxInvestmentsPerUser (${plan.maxInvestmentsPerUser}).`); const c = await UserInvestment.countDocuments({user:user._id,plan:plan._id}); console.log(`[Foundry][INVEST] Usuário tem ${c} investimentos neste plano.`); if(c>=plan.maxInvestmentsPerUser)return res.status(400).json({m:`Limite de ${plan.maxInvestmentsPerUser} aquisição(ões) atingido.`});} //
        const balanceBefore = user.balance; user.balance -= plan.price_mt; console.log(`[Foundry][INVEST] Saldo do usuário ${user.name} atualizado para: ${user.balance}`); //
        const newInvestment = new UserInvestment({ user: user._id, plan: plan._id, planSnapshot: { name: plan.name, price_mt: plan.price_mt, daily_profit_mt: plan.daily_profit_mt, duration_days: plan.duration_days } }); console.log("[Foundry][INVEST] Tentando salvar UserInvestment..."); await newInvestment.save(); console.log("[Foundry][INVEST] UserInvestment salvo:", newInvestment._id); //
        console.log("[Foundry][INVEST] Criando transação de compra..."); await createTransactionEntry(user._id, 'plan_purchase', -plan.price_mt, `Compra do Plano: ${plan.name}`, 'completed', balanceBefore, user.balance, {relatedInvestment: newInvestment._id}); console.log("[Foundry][INVEST] Transação de compra criada."); //
        if (user.referredBy) { console.log(`[Foundry][INVEST] Usuário indicado por ${user.referredBy}. Verificando bônus.`); const r=await User.findById(user.referredBy); const sS=await getOrInitializeSystemSettings(); if(r&&sS.isReferralSystemActive&&sS.referralPlanPurchaseBonusPercentage>0){const bA=plan.price_mt*sS.referralPlanPurchaseBonusPercentage; if(bA>0){console.log(`[Foundry][INVEST] Bônus de ${bA.toFixed(2)} para ${r.name}.`); const rBB=r.balance;r.balance+=bA; console.log(`[Foundry][INVEST] Saldo do referente ${r.name} para ${r.balance}. Salvando...`); await r.save(); console.log("[Foundry][INVEST] Referente salvo. Criando transação de bônus..."); await createTransactionEntry(r._id,'referral_bonus_plan',bA,`Bônus indicação (compra por ${user.name})`,'completed',rBB,r.balance,{relatedUser:user._id}); console.log("[Foundry][INVEST] Transação bônus criada. Notificando referente..."); await createUserNotification(r._id,'Bônus de Indicação!',`Você recebeu ${bA.toFixed(2)} MT porque ${user.name} adquiriu o plano ${plan.name}.`,'success','/referrals');}}} //
        if (!user.canWithdrawBonus) { console.log("[Foundry][INVEST] Habilitando saque de bônus para o usuário."); user.canWithdrawBonus = true; } //
        console.log(`[Foundry][INVEST] Salvando usuário ${user.name} final...`); await user.save(); console.log("[Foundry][INVEST] Usuário salvo. Notificando investimento..."); //
        await createUserNotification(user._id, 'Investimento Realizado com Sucesso!', `Você investiu no plano ${plan.name}. Acompanhe seus lucros!`, 'success', '/investments/my-history'); console.log("[Foundry][INVEST] Investimento concluído."); //
        res.status(201).json({ m: "Investimento realizado com sucesso!", investment: newInvestment }); //
    } catch (error) { console.error("[Foundry][INVEST] ERRO CRÍTICO ao realizar investimento:", error.message, error.stack); res.status(500).json({ m: "Erro no servidor ao tentar realizar o investimento. Por favor, contacte o suporte." }); } //
});
investmentRouter.get('/my-active', protectRoute, async (req, res) => { /* ... como antes ... */  //
    ...
    } catch (error) { console.error("[Foundry] Erro ao buscar investimento ativo:", error); res.status(500).json({ m: "Erro ao buscar seu investimento ativo." }); } //
});
investmentRouter.post('/collect-profit', protectRoute, async (req, res) => { /* ... como antes ... */  //
    try {
    ...
    } catch(error) { console.error("[Foundry] Erro ao coletar lucros:", error); res.status(500).json({m:"Erro no servidor ao tentar coletar lucros."}); } //
});
investmentRouter.get('/my-history', protectRoute, async (req, res) => { /* ... como antes ... */  //
    try {
        const page = parseInt(req.query.page) || 1; const limit = parseInt(req.query.limit) || 10; const skip = (page - 1) * limit; //
        ...
        const totalInvestments = await UserInvestment.countDocuments(query); //
        res.json({ investments: investments, currentPage: page, totalPages: Math.ceil(totalInvestments / limit), totalCount: totalInvestments }); //
    } catch (error) { console.error("[Foundry] Erro ao buscar histórico de investimentos:", error); res.status(500).json({ m: "Erro ao buscar seu histórico de investimentos." }); } //
});
...
async function startServer() { //
    ...
    try { //
        ...
        console.error('Falha Crítica ao Iniciar Servidor:', error.message, error.stack); // Adicionado error.stack
        process.exit(1); //
    }
}
