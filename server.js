...
const DEFAULT_MIN_WITHDRAWAL = parseFloat(process.env.DEFAULT_MIN_WITHDRAWAL) || 100; // Mantido conforme seu código
...
const UserSchema = new mongoose.Schema({
    ...
    // Campos para OTLT serão adicionados em uma próxima etapa se solicitado
...
UserSchema.methods.createPasswordResetToken = function() {
    const resetToken = crypto.randomBytes(32).toString('hex'); // Mantido o seu tamanho original
...
UserInvestmentSchema.pre('save', function(next) { 
    // ATUALIZADO: Garante que endDate e nextCollectionAvailableAt são definidos se for um novo investimento
    // E se os campos necessários do planSnapshot estiverem presentes.
    if (this.isNew) {
        if (this.planSnapshot && typeof this.planSnapshot.duration_days === 'number' && this.startDate) {
            ...
            this.lastProfitCalculationTime = this.startDate; 
             ...
             // Isso indica um problema se o planSnapshot ou startDate não estiverem corretos ao criar o UserInvestment
             console.error("[UserInvestment PRE-SAVE HOOK] Não foi possível calcular endDate ou nextCollectionAvailableAt. Verifique planSnapshot e startDate.");
             // Se endDate é 'required: true', o Mongoose irá barrar o save aqui se this.endDate permanecer undefined.
             // Não é necessário lançar um erro explícito aqui, a validação do schema fará isso.
        }
    }
    next();
});
...
// -----------------------------------------------------------------------------
// (protectRoute, adminOnly, getOrInitializeSystemSettings, createInitialAdmin, 
// createTransactionEntry, createUserNotification, updateUncollectedProfits como definidos anteriormente)
// Estas funções auxiliares foram mantidas como na sua última versão completa, com os nomes "Foundry" onde aplicável.
...
const adminOnly = async (req, res, next) => { 
    ...
    } catch(error) { console.error("[Foundry] Erro na verificação de admin:", error); res.status(500).json({ message: "Erro ao verificar permissões."}); }
};
    ...
    try {
        ...
        if (!settings) {
            ...
            settings = new SystemSettings({
                ...
                bonusWithdrawalRequiresPlan: true // Novo campo
            });
            await settings.save();
...
authRouter.post('/register', async (req, res) => { /* ... Mantido como no seu original ... */ 
    try {
...
authRouter.post('/login', async (req, res) => { /* ... Mantido como no seu original ... */ 
    try {
...
authRouter.post('/recover/request-question', async (req, res) => { /* ... Mantido como no seu original ... */ 
...
adminRouter.delete('/plans/:id', async (req, res) => { 
    try {
        ...
        const deletedPlan = await Plan.findByIdAndDelete(req.params.id); // Mudado para delete real, conforme solicitado
...
adminRouter.post('/users/:userId/assign-plan', async (req, res) => { /* ... como antes ... */ 
    try {
        ...
        const systemSettings = await getOrInitializeSystemSettings(); // Pega as configurações
        if (userInvestmentCount > 0 && !user.canWithdrawBonus && systemSettings.bonusWithdrawalRequiresPlan) { 
...
adminRouter.post('/users/:userId/initiate-password-reset', async (req, res) => {
    try {
        ...
        res.json({m: `Processo de redefinição de senha iniciado para ${user.email}. O token (para ser enviado ao usuário) é: ${resetToken}`});
    } catch (error) { console.error("[Foundry] Erro ao iniciar reset de senha pelo admin:", error); res.status(500).json({m: "Erro no servidor."}); }
});

// Outras rotas de admin (/api/admin/*) como na versão anterior
...
adminRouter.put('/users/:id/update-details',async(req,res)=>{ /* ... como antes ... */  
    try {
        ...
        if (typeof canWithdrawBonus === 'boolean') user.canWithdrawBonus = canWithdrawBonus;
...
// Rotas Admin para Contadores Regressivos (Integradas no adminRouter)
...
publicPlanRouter.get('/:id',async(req,res)=>{ try { if(!mongoose.Types.ObjectId.isValid(req.params.id)) return res.status(400).json({m:"ID de plano inválido."}); const plan = await Plan.findById(req.params.id); if (!plan || !plan.isActive) return res.status(404).json({ m: "Plano não encontrado ou inativo." }); res.json(plan); } catch (error) { console.error("[Foundry] Erro ao buscar plano por ID (público):", error); res.status(500).json({ m: "Erro ao buscar plano." }); } });
app.use('/api/plans', publicPlanRouter);

const publicDepositMethodRouter = express.Router();
publicDepositMethodRouter.get('/', async(req,res)=>{ try { const methods = await DepositMethod.find({ isActive: true }).select('-createdAt -updatedAt -__v -accountDetailsSchema'); res.json(methods); } catch (error) { console.error("[Foundry] Erro ao buscar métodos de depósito (público):", error); res.status(500).json({ m: "Erro ao buscar métodos de depósito." }); } });
app.use('/api/deposit-methods', publicDepositMethodRouter); 

const blogRouter = express.Router();
blogRouter.post('/', protectRoute, adminOnly, async (req, res) => { /* ... como antes ... */ try { const { title, content, slug, snippet, tags, isPublished, coverImageUrl } = req.body; if (!title || !content) return res.status(400).json({m: "Título e conteúdo são obrigatórios para o post."}); let postSlug = slug; if (!postSlug || postSlug.trim() === '') { postSlug = title.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]+/g, ''); } const existingSlug = await BlogPost.findOne({ slug: postSlug }); if (existingSlug && (!req.params.id || existingSlug._id.toString() !== req.params.id)) return res.status(400).json({m: "Este slug já está em uso. Escolha outro."}); const newPost = new BlogPost({ title, content, slug: postSlug, snippet: snippet || (content.length > 250 ? content.substring(0, 250) + '...' : content), tags: Array.isArray(tags) ? tags : (tags ? tags.split(',').map(t=>t.trim()).filter(t => t) : []), isPublished: isPublished === true, coverImageUrl, author: req.user.id }); await newPost.save(); res.status(201).json({ m: "Post do blog criado com sucesso!", post: newPost }); } catch (error) { console.error("[Foundry] Erro ao criar post:", error); if (error.code === 11000) return res.status(400).json({m: "Um post com este título ou slug já existe."}); if (error.name === 'ValidationError') return res.status(400).json({m: "Erro de validação.", e: Object.values(error.errors).map(val => val.message)}); res.status(500).json({ m: "Erro no servidor ao criar post." }); } });
blogRouter.get('/', async (req, res) => { /* ... como antes (rota pública) ... */ try { const page = parseInt(req.query.page) || 1; const limit = parseInt(req.query.limit) || 10; const skip = (page - 1) * limit; const tagFilter = req.query.tag; const searchQuery = req.query.search; let query = { isPublished: true }; if (tagFilter) query.tags = tagFilter.trim().toLowerCase(); if (searchQuery) query.title = { $regex: searchQuery, $options: 'i' };  const posts = await BlogPost.find(query).populate('author', 'name').sort({ createdAt: -1 }).skip(skip).limit(limit).select('title slug snippet tags createdAt coverImageUrl author views');  const totalPosts = await BlogPost.countDocuments(query); res.json({ posts: posts, currentPage: page, totalPages: Math.ceil(totalPosts / limit), totalCount: totalPosts }); } catch (error) { console.error("[Foundry] Erro ao buscar posts (público):", error); res.status(500).json({ m: "Erro ao buscar posts do blog." }); } });
blogRouter.get('/all', protectRoute, adminOnly, async (req, res) => { /* ... como antes (rota admin) ... */ try { const page = parseInt(req.query.page) || 1; const limit = parseInt(req.query.limit) || 20; const skip = (page - 1) * limit; const { isPublished, search } = req.query; let query = {}; if (isPublished !== undefined) query.isPublished = (isPublished === 'true'); if (search) query.title = { $regex: search, $options: 'i' }; const posts = await BlogPost.find(query).populate('author', 'name').sort({ createdAt: -1 }).skip(skip).limit(limit); const totalPosts = await BlogPost.countDocuments(query); res.json({ posts: posts, currentPage: page, totalPages: Math.ceil(totalPosts / limit), totalCount: totalPosts }); } catch (error) { console.error("[Foundry] Erro ao buscar todos os posts (admin):", error); res.status(500).json({ m: "Erro ao buscar posts." }); } });
blogRouter.get('/slug/:slug', async (req, res) => { /* ... como antes (rota pública) ... */ try { const post = await BlogPost.findOneAndUpdate( { slug: req.params.slug.toLowerCase(), isPublished: true }, { $inc: { views: 1 } }, { new: true } ).populate('author', 'name'); if (!post) return res.status(404).json({ m: "Post do blog não encontrado ou não publicado." }); res.json(post); } catch (error) { console.error("[Foundry] Erro ao buscar post por slug:", error); res.status(500).json({ m: "Erro ao buscar post." }); } });
blogRouter.get('/id/:id', protectRoute, adminOnly, async (req, res) => { /* ... como antes (rota admin) ... */ try { if(!mongoose.Types.ObjectId.isValid(req.params.id)) return res.status(400).json({m:"ID Inv."}); const post = await BlogPost.findById(req.params.id).populate('author', 'name'); if (!post) return res.status(404).json({m: "Não enc."}); res.json(post); } catch (error) { console.error("[Foundry] Erro ao buscar post por ID (admin):", error); res.status(500).json({ m: "Erro ao buscar post." }); } });
blogRouter.put('/:id', protectRoute, adminOnly, async (req, res) => { /* ... como antes ... */ try { if(!mongoose.Types.ObjectId.isValid(req.params.id)) return res.status(400).json({m:"ID Inv."}); const updateData = { ...req.body }; if (updateData.slug) { const newSlug = updateData.slug.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]+/g, ''); const existingPostWithSlug = await BlogPost.findOne({ slug: newSlug, _id: { $ne: req.params.id } }); if (existingPostWithSlug) return res.status(400).json({m: "Este slug já está em uso por outro post."}); updateData.slug = newSlug; } else if (updateData.title && !updateData.slug) { updateData.slug = updateData.title.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]+/g, ''); } if (updateData.content && updateData.snippet === undefined) { updateData.snippet = updateData.content.substring(0, 250) + (updateData.content.length > 250 ? '...' : ''); } updateData.updatedAt = Date.now(); const post = await BlogPost.findByIdAndUpdate(req.params.id, { $set: updateData }, { new: true, runValidators: true }); if (!post) return res.status(404).json({m: "Não enc."}); res.json({ m: "Post atualizado!", post: post }); } catch (error) { console.error("[Foundry] Erro ao atualizar post:", error); if (error.code === 11000) return res.status(400).json({m: "Título/Slug já existe."}); if (error.name === 'ValidationError') return res.status(400).json({m: "Validação.", e: Object.values(error.errors).map(v=>v.message)}); res.status(500).json({ m: "Erro." }) } });
blogRouter.delete('/:id', protectRoute, adminOnly, async (req, res) => { /* ... como antes ... */ try { if(!mongoose.Types.ObjectId.isValid(req.params.id)) return res.status(400).json({m:"ID Inv."}); const post = await BlogPost.findByIdAndDelete(req.params.id); if (!post) return res.status(404).json({m: "Não enc."}); res.json({ m: "Post deletado." }); } catch (e) { console.error("[Foundry] Erro ao deletar post:", e); res.status(500).json({m:"Erro."}) } });
app.use('/api/blog', blogRouter);

const promotionRouter = express.Router();
promotionRouter.post('/', protectRoute, adminOnly, async (req, res) => { /* ... como antes ... */ try { const { title, description } = req.body; if (!title || !description) return res.status(400).json({m: "Título/Descrição obrigatórios."}); const newPromotionData = { ...req.body, isActive: req.body.isActive === true, startDate: req.body.startDate ? new Date(req.body.startDate) : Date.now(), endDate: req.body.endDate ? new Date(req.body.endDate) : null, countdownTargetDate: req.body.countdownTargetDate ? new Date(req.body.countdownTargetDate) : null }; const newPromotion = new Promotion(newPromotionData); await newPromotion.save(); res.status(201).json({ m: "Promoção criada!", promotion: newPromotion }); } catch (e) { console.error("[Foundry] Erro ao criar promoção:", e); if (e.name === 'ValidationError') return res.status(400).json({m: "Validação.", e: Object.values(e.errors).map(v=>v.message)}); res.status(500).json({ m: "Erro." }) } });
promotionRouter.get('/active', async (req, res) => { /* ... como antes ... */ try { const now = new Date(); const promotions = await Promotion.find({ isActive: true, $or: [ { startDate: { $lte: now } }, { startDate: null } ], $or: [ { endDate: { $gte: now } }, { endDate: null } ] }).sort({ priority: -1, createdAt: -1 });  res.json(promotions); } catch (e) { console.error("[Foundry] Erro ao buscar promoções ativas:", e); res.status(500).json({ m: "Erro." }) } });
promotionRouter.get('/type/:typeName', async (req, res) => { 
    try { 
...
promotionRouter.get('/all', protectRoute, adminOnly, async (req, res) => { /* ... como antes ... */ try { const page = parseInt(req.query.page) || 1; const limit = parseInt(req.query.limit) || 10; const skip = (page - 1) * limit; const isActiveFilter = req.query.isActive; let query = {}; if (isActiveFilter !== undefined) { query.isActive = (isActiveFilter === 'true'); } const promotions = await Promotion.find(query).sort({ createdAt: -1 }).skip(skip).limit(limit); const totalPromotions = await Promotion.countDocuments(query); res.json({ promotions: promotions, currentPage: page, totalPages: Math.ceil(totalPromotions / limit), totalCount: totalPromotions }); } catch (e) { console.error("[Foundry] Erro ao buscar todas promoções (admin):", e); res.status(500).json({ m: "Erro." }) } });
promotionRouter.get('/:id', protectRoute, adminOnly, async (req, res) => { /* ... como antes ... */ try { if(!mongoose.Types.ObjectId.isValid(req.params.id))return res.status(400).json({m:"ID Inv."}); const promotion = await Promotion.findById(req.params.id); if (!promotion) return res.status(404).json({m: "Não enc."}); res.json(promotion); } catch (e) { console.error("[Foundry] Erro ao buscar promoção por ID (admin):", e); res.status(500).json({m:"Erro."}) }});
promotionRouter.put('/:id', protectRoute, adminOnly, async (req, res) => { /* ... como antes ... */ try { if(!mongoose.Types.ObjectId.isValid(req.params.id))return res.status(400).json({m:"ID Inv."}); const updateData = { ...req.body }; if (updateData.isActive !== undefined) { updateData.isActive = (updateData.isActive === true || updateData.isActive === 'true'); } if (updateData.startDate) updateData.startDate = new Date(updateData.startDate); if (updateData.endDate) updateData.endDate = new Date(updateData.endDate); else if (updateData.endDate === '') updateData.endDate = null;  if (updateData.countdownTargetDate) updateData.countdownTargetDate = new Date(updateData.countdownTargetDate); else if (updateData.countdownTargetDate === '') updateData.countdownTargetDate = null; const updatedPromotion = await Promotion.findByIdAndUpdate(req.params.id, { $set: updateData }, { new: true, runValidators: true }); if (!updatedPromotion) return res.status(404).json({m: "Não enc."}); res.json({ m: "Promoção atualizada!", promotion: updatedPromotion }); } catch (e) { console.error("[Foundry] Erro ao atualizar promoção:", e); if (e.name === 'ValidationError') return res.status(400).json({m: "Validação.", e: Object.values(e.errors).map(val => val.message)}); res.status(500).json({ m: "Erro." }) } });
...
app.use('/api/countdowns', countdownRouter); // Monta aqui para rotas públicas (como /active)
app.use('/api/admin/countdowns', protectRoute, adminOnly, countdownRouter); // Monta de novo, mas com proteção de admin para todas as rotas do router
...
async function startServer() {
    ...
    try {
        ...
        console.error('Falha Crítica ao Iniciar Servidor:', error.message, error.stack); 
        process.exit(1);
    }
}
